...
const valoresFaturamento = faturamentoDiario.filter(dia => dia.valor > 0).map(dia => dia.valor);
const menorFaturamento = Math.min(...valoresFaturamento);
const maiorFaturamento = Math.max(...valoresFaturamento);
const mediaFaturamento = valoresFaturamento.reduce((a, b) => a + b, 0) / valoresFaturamento.length;
